...
1. Overdue tasks are loaded correctly after system restart (Task 6.2)
2. Overdue tasks are executed immediately (Task 6.3)
3. Logs are recorded correctly in activity_logs (Task 6.4)
4. Future tasks continue to work properly (preservation check)
5. Multiple overdue tasks are processed in correct order
"""
...
async def verify_overdue_tasks_loading():
    """Task 6.2: Verify that overdue tasks are loaded correctly."""
    print("\n=== Test 1: Overdue Tasks Loading (Task 6.2) ===")
    
    # Create test data
    async with get_connection() as db:
        await db.execute("""
            INSERT INTO userbots (id, session_file, status, joins_today, joins_reset_at, created_at, updated_at)
            VALUES (900001, 'verify_load.session', 'active', 0, datetime('now', '+1 day'), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        
        await db.execute("""
            INSERT INTO chats (id, chat_link, status, created_at, updated_at)
            VALUES (900001, 'https://t.me/verify_load', 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
    ...
    print("✓ Test 1 PASSED: Overdue tasks are loaded correctly\n")
    return True


async def verify_overdue_tasks_execution():
    """Task 6.3: Verify that overdue tasks are executed immediately."""
    print("=== Test 2: Overdue Tasks Execution (Task 6.3) ===")
    
    # Create test data
    async with get_connection() as db:
        await db.execute("""
            INSERT OR REPLACE INTO userbots (id, session_file, status, joins_today, joins_reset_at, created_at, updated_at)
            VALUES (900002, 'verify_exec.session', 'active', 0, datetime('now', '+1 day'), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        
        await db.execute("""
            INSERT OR REPLACE INTO chats (id, chat_link, status, created_at, updated_at)
            VALUES (900002, 'https://t.me/verify_exec', 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        ...
        """, (scheduled_at.isoformat(), (now - timedelta(hours=1)).isoformat()))
    ...
    # Try to get tasks from queue - should return immediately
    # Note: There may be other overdue tasks in the database, so we need to
    # keep getting tasks until we find ours or timeout
    start_time = datetime.now(timezone.utc)
    task = None
    found_our_task = False
    
    for _ in range(50):  # Try up to 50 times
        try:
            t = await asyncio.wait_for(queue.get_next_task(), timeout=2.0)
            if t and t.task_id == 900002:
                task = t
                found_our_task = True
                break
    ...
    if not found_our_task:
        print("✗ FAILED: Our test task 900002 was not returned")
        return False
    
    wait_time = (end_time - start_time).total_seconds()
    
    # Should return relatively quickly (< 10 seconds even with other tasks)
    if wait_time > 10.0:
        print(f"✗ FAILED: Task took {wait_time:.2f}s to return (expected < 10s)")
        return False
    
    print(f"✓ Overdue task returned ({wait_time:.3f}s)")
    ...
    async with get_connection() as db:
        cursor = await db.execute(
            "SELECT status FROM join_tasks WHERE id = 900002"
        )
        row = await cursor.fetchone()
    ...
    print("✓ Test 2 PASSED: Overdue tasks are executed immediately\n")
    return True


async def verify_logging():
    """Task 6.4: Verify that logs are recorded correctly."""
    print("=== Test 3: Logging Verification (Task 6.4) ===")
    ...
    print("✓ Test 3 PASSED: Logging works correctly\n")
    return True


async def verify_future_tasks():
    """Verify that future tasks still work properly (preservation check)."""
    print("=== Test 4: Future Tasks Preservation ===")
    
    # Create test data
    async with get_connection() as db:
        await db.execute("""
            INSERT OR REPLACE INTO userbots (id, session_file, status, joins_today, joins_reset_at, created_at, updated_at)
            VALUES (900003, 'verify_future.session', 'active', 0, datetime('now', '+1 day'), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        
        await db.execute("""
            INSERT OR REPLACE INTO chats (id, chat_link, status, created_at, updated_at)
            VALUES (900003, 'https://t.me/verify_future', 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        ...
        await db.execute("""
            INSERT INTO join_tasks (id, userbot_id, chat_id, status, scheduled_at, created_at)
            VALUES (900003, 900003, 900003, 'pending', ?, ?)
    ...
    queue = JoinQueue()
    await queue.add_task(900003, 900003, 900003, scheduled_at)
    ...
    try:
        ...
        print("✓ Test 4 PASSED: Future tasks work correctly\n")
        return True
...
async def verify_multiple_overdue_tasks():
    ...
    print("=== Test 5: Multiple Overdue Tasks Order ===")
    
    # Create test data
    async with get_connection() as db:
        await db.execute("""
            INSERT OR REPLACE INTO userbots (id, session_file, status, joins_today, joins_reset_at, created_at, updated_at)
            VALUES (900004, 'verify_multi.session', 'active', 0, datetime('now', '+1 day'), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        
        await db.execute("""
            INSERT OR REPLACE INTO chats (id, chat_link, status, created_at, updated_at)
            VALUES (900004, 'https://t.me/verify_multi', 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
    ...
    print("✓ Test 5 PASSED: Multiple overdue tasks processed correctly\n")
    return True
...
async def main():
    ...
    try:
        # Run all tests
        results.append(("Task 6.2: Overdue Tasks Loading", await verify_overdue_tasks_loading()))
        results.append(("Task 6.3: Overdue Tasks Execution", await verify_overdue_tasks_execution()))
        results.append(("Task 6.4: Logging", await verify_logging()))
