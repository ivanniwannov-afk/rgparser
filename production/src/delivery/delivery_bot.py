...
class DeliveryBot:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # Start polling to listen for callback button presses
...
"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    # Get wait time from the exception
                    wait_time = e.retry_after + 1  # Add 1 second buffer
                    logger.warning(f"RetryAfter {wait_time}s, waiting before retry...")
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(f"Failed to deliver lead after {max_retries} retries")
                    raise
    ...
    async def handle_spam_feedback(self, message_text: str) -> None:
        ...
        """
        # Use shared write lock to prevent concurrent write conflicts
