...
class UserbotPoolManager:
    ...
    **Validates: Requirements 12.1, 12.2, 12.3, 12.4, 13.1, 13.2, 16.1, 16.3, 16.4**
    ...
    async def add_userbot(self, session_file: str) -> int:
        ...
        **Validates: Requirements 12.1, 12.3, 2.7**
        ...
        # Add to database using UPSERT to handle duplicates on system restart
        async with database.get_connection() as db:
            ...
            if existing:
                # Userbot already exists, return existing ID
    ...
    async def get_client(self, userbot_id: int) -> Optional['Client']:
        """Get Pyrogram client for a userbot.
        
        **Validates: Requirements 2.3**
        
        Attempts to start the Pyrogram client for the specified userbot.
        If the client fails to start (banned account, corrupted session),
        marks the userbot as 'banned' in the database.
        
        Args:
            userbot_id: ID of the userbot
            
        Returns:
            Pyrogram Client instance if successful, None if failed
        """
        ...
        try:
            # Extract session name from session file path
            session_name = Path(userbot.session_file).stem
            ...
            # Client failed to start - mark as banned
            await self._log_activity(
                "UserbotPoolManager",
                "ERROR",
                f"Failed to start client for userbot {userbot_id}: {str(e)}",
                {"userbot_id": userbot_id, "error": str(e)}
            )
            ...
            async with database.get_connection() as db:
                await db.execute(
                    """UPDATE userbots SET status = ?, updated_at = CURRENT_TIMESTAMP
                       WHERE id = ?""",
                    (UserbotStatus.BANNED.value, userbot_id)
                )
                await db.commit()
